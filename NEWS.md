# workflowtools 0.0.18

* `archive_extract_once()` now forces a binary-safe `encoding` while it reads and extracts. `archive::archive()` and `archive::archive_extract()` open the archive with `file(path, "rb")`, which takes its encoding from `getOption("encoding")`; under `options(encoding = "UTF-8")` a re-encoding layer is applied to a **binary** stream, libarchive misparses the ZIP, and every member's recorded size comes back as `0`. Sizes are exactly what this function compares against, so under that option every file looks short: the archive was re-extracted on **every single call**, and the post-extraction verification could never pass no matter what landed on disk. `options(encoding = "UTF-8")` is in force inside a SpaDES run, which is why this only ever bit in a pipeline and never in a standalone test. The misparse is also size-dependent -- a 929-byte zip reads correctly under either encoding while a 1.26 GB one returns all zeros -- so a small fixture cannot reproduce it; the regression test asserts the guard directly instead. Worth reporting upstream: `archive` should open its connections with `encoding = "native.enc"`;
